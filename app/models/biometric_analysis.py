...
class BiometricAnalysis(db.Model):
    ...
    gender = db.Column(
        db.String(10), nullable=False, comment="Gender: 'male', 'female', or 'other'"
    )
    ...
    hip = db.Column(
        db.Float, nullable=True, comment="Hip circumference in cm (optional for males)"
    )

    # ========== 🔥 BILATERAL MUSCLE MEASUREMENTS (Optional) ==========
    # Biceps (bíceps)
    biceps_left = db.Column(
        db.Float, nullable=True, comment="Left biceps circumference in cm"
    )
    biceps_right = db.Column(
        db.Float, nullable=True, comment="Right biceps circumference in cm"
    )

    # Thighs / Quadriceps (muslos/cuádriceps)
    thigh_left = db.Column(
        db.Float, nullable=True, comment="Left thigh (quadriceps) circumference in cm"
    )
    thigh_right = db.Column(
        db.Float, nullable=True, comment="Right thigh (quadriceps) circumference in cm"
    )

    # Calves (gemelos/pantorrillas)
    calf_left = db.Column(
        db.Float, nullable=True, comment="Left calf circumference in cm"
    )
    calf_right = db.Column(
        db.Float, nullable=True, comment="Right calf circumference in cm"
    )

    # ========== ACTIVITY DATA ==========
    activity_factor = db.Column(
        db.Float, nullable=True, comment="Activity multiplier (1.2-1.9)"
    )
    ...
    goal = db.Column(
        db.String(32), nullable=True, comment="Goal: lose_weight, maintain, gain_muscle"
    )
    ...
    waist_height_ratio = db.Column(
        db.Float, nullable=True, comment="Waist-to-Height Ratio"
    )
    metabolic_age = db.Column(
        db.Float, nullable=True, comment="Estimated metabolic age"
    )

    # ========== NUTRITION TARGETS ==========
    maintenance_calories = db.Column(
        db.Float, nullable=True, comment="Daily maintenance calories"
    )
    protein_grams = db.Column(
        db.Float, nullable=True, comment="Daily protein target in grams"
    )
    carbs_grams = db.Column(
        db.Float, nullable=True, comment="Daily carbs target in grams"
    )
    fats_grams = db.Column(
        db.Float, nullable=True, comment="Daily fats target in grams"
    )
