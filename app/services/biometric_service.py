...
def create_analysis(
	...
	try:
		...
		missing_fields = [field for field in required_fields if field not in biometric_data]
		...
		logger.info(f"Created biometric analysis ID={analysis.id} for user_id={user_id}")
		...
		if request_fitmaster:
			...
			if fitmaster_error:
				logger.warning(f"FitMaster analysis failed for ID={analysis.id}: {fitmaster_error}")
...
def get_user_analyses(user_id: int, limit: int = 10) -> list:
	...
	"""
	return BiometricAnalysis.query.filter_by(user_id = user_id) \
...
def update_fitmaster_data(analysis_id: int, fitmaster_data: Dict) -> Tuple[bool, Optional[str]]:
	"""
