...
def send_message():
    ...
    analyses = (
        BiometricAnalysis.query.filter_by(user_id=current_user.id)
        .order_by(BiometricAnalysis.created_at.desc())
        .all()
    )
    form.analysis_id.choices = [(0, "Ninguno (consulta general)")] + [
        (a.id, f'Análisis #{a.id} - {a.created_at.strftime("%d/%m/%Y")}')
        for a in analyses
    ]
