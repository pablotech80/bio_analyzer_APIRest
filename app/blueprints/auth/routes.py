...
def register():
    ...
    if form.validate_on_submit():
        try:
            ...
            verify_url = url_for(
                "auth.verify_email",
                token=user_updated.verification_token,
                _external=True,
            )
...
def reset_password(token):
    ...
    if form.validate_on_submit():
        user = AuthService.reset_password_with_token(
            token=token, new_password=form.password.data
        )

        if user:
            flash(
                "Tu contraseña ha sido reseteada exitosamente. Ya puedes iniciar sesión.",
                "success",
            )
            return redirect(url_for("auth.login"))
        else:
            flash(
                "El link de reseteo es inválido o ha expirado. Solicita uno nuevo.",
                "danger",
            )
...
def api_login():
    ...
    if not user:
        return (
            jsonify({"success": False, "error": "Email o contraseña incorrectos"}),
            401,
        )
...
def resend_verification():
    ...
    try:
        ...
        flash(
            f"Se ha generado un nuevo link de verificación. " f"(Demo: {verify_url})",
            "info",
        )
    except ValueError as e:
        flash(str(e), "warning")
