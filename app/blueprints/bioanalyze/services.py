...
def run_biometric_analysis(form: MultiDict[str, str]) -> AnalysisPayload:
    ...
    carbohidratos_kg = _parse_positive_float(carbohidratos_kg_str, "carbohidratos_kg") if carbohidratos_kg_str else None
...
def build_interpretations_for_record(record: BiometricAnalysis) -> Dict[str, Any]:
    """Regenerate interpretations for a stored analysis record."""

    genero = convertir_genero(record.gender)

    interpretaciones = {
        "imc": interpretar_imc(record.bmi, record.ffmi, genero),
        "porcentaje_grasa": interpretar_porcentaje_grasa(record.body_fat_percentage, genero),
        "ffmi": interpretar_ffmi(record.ffmi, genero),
        "rcc": interpretar_rcc(record.waist_hip_ratio, genero)
        if genero == Sexo.MUJER and record.waist_hip_ratio is not None
        else None,
        "ratio_cintura_altura": interpretar_ratio_cintura_altura(record.waist_height_ratio),
        "edad_metabolica": interpretar_edad_metabolica_avanzada(
            record.age,
